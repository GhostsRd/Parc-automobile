            ...
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-xl border border-gray-100">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 border-b border-gray-100">
                    <div class="p-6 border-r border-gray-100">
                        ...
                        <p class="text-2xl font-black text-gray-900">{{ number_format($maintenances->sum('cout'), 2,
                            ',', ' ') }} €</p>
                    </div>
                    <div class="p-6 border-r border-gray-100">
                    ...
                    <div class="p-6">
                        ...
                        <p class="text-2xl font-black text-gray-900">
                            {{ $maintenances->max('date_intervention') ?
                            \Carbon\Carbon::parse($maintenances->max('date_intervention'))->format('d/m/Y') : '--' }}
                        </p>
                    </div>
                </div>
                    ...
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="bg-gray-50/50">
                                ...
                                <th class="px-6 py-4 text-xs font-bold text-gray-500 uppercase tracking-wider">Véhicule
                                </th>
                                <th class="px-6 py-4 text-xs font-bold text-gray-500 uppercase tracking-wider">Type /
                                    Note</th>
                        ...
                        <tbody class="divide-y divide-gray-100">
                            ...
                            <tr class="hover:bg-gray-50/80 transition">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex flex-col">
                                        <span class="text-sm font-bold text-indigo-600">{{ $maintenance->vehicle->brand
                                            }} {{ $maintenance->vehicle->model }}</span>
                                        <span class="text-xs font-mono text-gray-500">{{
                                            $maintenance->vehicle->immatriculation }}</span>
                                    </div>
                                </td>
                                <td class="px-6 py-4">
                                    <div class="flex items-center">
                                        <span
                                            class="px-2 py-1 text-xs font-extrabold rounded-md {{ $maintenance->type == 'vidange' ? 'bg-fuchsia-100 text-fuchsia-700' : 'bg-blue-100 text-blue-700' }} uppercase">
                                            {{ $maintenance->type }}
                                        </span>
                                        <span class="ml-3 text-sm text-gray-600 truncate max-w-xs">{{
                                            Str::limit($maintenance->notes, 40) }}</span>
                                    </div>
                                </td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span class="text-sm font-black text-gray-900">{{ number_format($maintenance->cout,
                                        2, ',', ' ') }} €</span>
                                </td>
                            ...
                            <tr>
                                <td colspan="6" class="px-6 py-12 text-center">
                                    <p class="text-gray-400 italic text-sm">Aucun entretien enregistré dans la base de
                                        données.</p>
                                </td>
                            </tr>
                            @endforelse
